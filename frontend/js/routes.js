        ...
        if($ref == 'accept'){
                ...
                success : function(res){
                    ...
                    if(data[0].duel_questions != null){
                        if(data[0].duel_questions.split('/').length <= 1){
                            ...
                            setTimeout(function() { hide_timer(); }, 500);
                        }
                        
                        ids_questions_duel = data[0].duel_questions.split('/');
                    }else{
                        // message('Ocurrio un error con el duelo, verifique su conexion a internet...!');
            ...
            setTimeout(function(){
                ...
                if(questions_duel_.length == 0){
                    // location.reload();
                    //message('No se cargaron las preguntas correctamente, verifique su conección a internet');
                }else{
...
function load_questions(filter,ref){
    ...
    $.ajax({
        ...
        success     : function(res){
            ...
            // events to menu question
            
            // click to deactivate question
                ...
                $.ajax({
                    ...
                    success : function(res){
                        ...
                        $('.structure .multiple_choice_edit_case .input img').unbind('click').click(function(){
                            evt_append_question_edit($(this).parent().find('input').val());
                        });
                        
                        // input text event to insert new field in list options
                        $('.structure .multiple_choice_edit_case .input input').unbind('keyup').keyup(function(e){
                            if(e.keyCode == 13)
                                evt_append_question_choice_edit($(this).val());
                        });
                        ...
                        $('.structure .order_questions_edit_case .input input').unbind('keyup').keyup(function(e){
                            if(e.keyCode == 13)
                                evt_append_question_edit($(this).val());
                        });
                        
                        // click to remove option from list options
                        ...
                        // click to remove option from list options
                        $('#sortable_edit_case li img').unbind('click').click(function(e) {
                            $(this).parent().remove();
    ...
    $.ajax({
        ...
        success                 : function(res){
            ...
            if(data.status == "OK"){
                ...
                setTimeout(function(){ message(data.message); },1000);
            }
            $('.loader').fadeOut(500);
        }
    });
...
function evt_specialty(param, $id){
    ...
    // ajax to acivate or deactivate specialty
    $.ajax({
                    ...
                    $('.start_race .content_question input[type=radio],.start_race .wrapper .content_start_game .content_question .sortable_answer + .container_btn button').unbind('click').click(function(){debugger
                        ...
                        if(type_question == 1){
                            ...
                            if(answer == correct_answer){
                                ...
                                setTimeout(function(){ question_status('.start_race','correct'); },1000);
                                        ...
                                        $.ajax({
                                            ...
                                            success : function(res){
                                                ...
                                                if(current_question_ + 1 < data.metadata.correct_answers){
                                                    ...
                                                    global_timeout =  setTimeout(function() { evt_next_question('.start_race'); },500);
                                                }
                                            }
                                        });
                                    }else{
...
function question_status(page_referer, state){
    ...
    if(page_referer != '.duel_users'){
        $(page_referer + ' .question_result .report_question').unbind('click').click(function(){
            ...
            $.mobile.changePage('#report_question',{role:'dialog',transition: 'slide'});
        });
    }
}
    ...
    if($(page_referer + ' .content_question.active').next().length == 0){
        ...
        setTimeout(function(){
            ...
            if(number_question == top_question + 1){
                ...
                current_content.fadeOut(50);
                
                $(page_referer + ' .wrapper > div').hide(10);
                $(page_referer + ' div[data-role="header"] .top_questions').hide(10);
...
function show_timer(time, page_referer){
    ...
    if(page_referer == "" || page_referer == null){
        ...
        if(cont){
            $('.question_time').fadeIn(500).find('em').text(time+'s');
